...
if __name__ == "__main__":
    ...
    # List the table class name, and only take the table with pokemon infos.
    table_class_name_list = []
    for table in root.xpath("//table"):
        table_class_name = table.get('class')
        table_class_name_list.append(table_class_name)
    table_class_name_array = np.array(table_class_name_list)
    table_class_name_array = np.unique(table_class_name_array)[2:10]
    # List the pokemon info in a line for each.
    for tbname in table_class_name_array:
